...
JetMatchingNewFxFx::JetMatchingNewFxFx(const edm::ParameterSet& iConfig){
  ...
  // Add some printout for debugging purposes
  std::cout << "Using the new FxFx matching procedure" << std::endl;
}

bool JetMatchingNewFxFx::initAfterBeams() {
...
bool JetMatchingNewFxFx::doVetoPartonLevelEarly(const Event& event) {
...
void JetMatchingNewFxFx::sortIncomingProcess(const Event &event){
...
bool JetMatchingNewFxFx::doVetoProcessLevel(Event& process) {
...
void JetMatchingNewFxFx::jetAlgorithmInput(const Event &event, int iType){
...
void JetMatchingNewFxFx::runJetAlgorithm(){; }

bool JetMatchingNewFxFx::doVetoStep(int iPos, int nISR, int nFSR, const Event& event){
...
void JetMatchingNewFxFx::setDJR( const Event& event) {
...
bool JetMatchingNewFxFx::matchPartonsToJets(int iType){
...
int JetMatchingNewFxFx::matchPartonsToJetsLight(){
  // Store things that are necessary to perform the kT-MLM veto externally.
...
int JetMatchingNewFxFx::matchPartonsToJetsHeavy(){
...
int JetMatchingNewFxFx::matchPartonsToJetsOther(){
  // Currently, heavy jets are unmatched
  // If there are no extra jets, then accept
  // jetMomenta is NEVER used by MadGraph and is always empty.
  //  This check does nothing.
...
bool JetMatchingNewFxFx::doShowerKtVeto(double pTfirst){
