...
L1Phase2MuDTShower::L1Phase2MuDTShower(
    int bx, int wh, int sc, int st, int sl, int ndigis, int avg_pos, int avg_time)
    : m_bx(bx),
      m_wheel(wh),
      m_sector(sc),
      m_station(st),
...
int L1Phase2MuDTShower::avg_time() const { return m_avg_time; }

int L1Phase2MuDTShower::avg_pos() const { return m_avg_pos; }