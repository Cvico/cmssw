...
muonTable = simpleCandidateFlatTableProducer.clone(
    ...
    variables = cms.PSet(CandVars,
        ...
        LepGood_pfRelIso03_chg = Var("pfIsolationR03().sumChargedHadronPt/pt",float,doc="PF relative isolation dR=0.3, charged component"),
        LepGood_pfRelIso03_all = Var("(pfIsolationR03().sumChargedHadronPt + max(pfIsolationR03().sumNeutralHadronEt + pfIsolationR03().sumPhotonEt - pfIsolationR03().sumPUPt/2,0.0))/pt",float,doc="PF relative isolation dR=0.3, total (deltaBeta corrections)"),
        LepGood_pfRelIso04_all = Var("(pfIsolationR04().sumChargedHadronPt + max(pfIsolationR04().sumNeutralHadronEt + pfIsolationR04().sumPhotonEt - pfIsolationR04().sumPUPt/2,0.0))/pt",float,doc="PF relative isolation dR=0.4, total (deltaBeta corrections)"),
        LepGood_miniRelIsoCharged = Var("userFloat('miniIsoChg')/pt",float),
        LepGood_miniRelIsoNeutral = Var("(userFloat('miniIsoAll')-userFloat('miniIsoChg'))/pt",float),
        ...
        LepGood_jetPNET = Var("?userCand('jetForLepJetVar').isNonnull()?max(userCand('jetForLepJetVar').bDiscriminator('pfParticleNetFromMiniAODAK4PuppiCentralDiscriminatorsJetTags:BvsAll'),0.0):0.0", float),
        LepGood_jetPtRatio = Var("?userCand('jetForLepJetVar').isNonnull()?min(userFloat('ptRatio'),1.5):1.0/(1.0+(pfIsolationR04().sumChargedHadronPt + max(pfIsolationR04().sumNeutralHadronEt + pfIsolationR04(\
).sumPhotonEt - pfIsolationR04().sumPUPt/2,0.0))/pt)",float),
        LepGood_dxy = Var("log(abs(dB('PV2D')))",float),
        LepGood_sip3d = Var("abs(dB('PV3D')/edB('PV3D'))",float),
        LepGood_dz = Var("log(abs(dB('PVDZ')))",float),
        LepGood_segmentComp = Var("segmentCompatibility",float),
        # =================================================================================== #
        ),
